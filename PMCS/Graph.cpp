...
void Graph::FindGraphDegreeCode()
{
	graphDegreeCode = 0;
	for(int i=0; i < num_vertices(G); i++)
	{
		if(isValidNode_Vb[i])
			graphDegreeCode += IGetDegree(NGetNodeWithID(i))*50;
			// graphDegreeCode += degreeInfo[IGetDegree(NGetNodeWithID(i))].second;
	}
	return;
}
...
// Print
void Graph::PrintFile_gSpan(char *fileName)
{
	ofstream outfs;
	...
	else
	{
		...
		for(boost::tie(ei, ei_end) = boost::edges(G); ei!=ei_end; ++ei)
		{
			outfs << "e " << v_index[boost::source(*ei, G)] << " " << v_index[boost::target(*ei, G)];
...
void Graph::DFS(int ori_source_id, vector<int> &visited, vector<int> &nodeIds, Graph *g)
{
	// int source_nodeId = g->IGetNodeID(start_n);
	int source_nodeId = nodeIds[ori_source_id];
	// cout << "DFS on " << ori_source_id  << " " << source_nodeId << endl;
	visited[ori_source_id] = 1;
	pair<adjacency_iterator, adjacency_iterator> vp = boost::adjacent_vertices(NGetNodeWithID(ori_source_id), G);
	while(vp.first != vp.second)
	{
		// cout << "Got on " << IGetNodeID(*vp.first) << endl;
		if(!BIsThisEdgeHidden(ori_source_id, IGetNodeID(*vp.first))){
			// cout << "NOT A HIDDEDN EDGE btw " << ori_source_id << " " << IGetNodeID(*vp.first) << endl;
			// PrintEdge(EGetEdge(ori_source_id, IGetNodeID(*vp.first)));
			int target_nodeId;
			if(!visited[IGetNodeID(*vp.first)])
			{	
				target_nodeId = g->BInsertNode(IGetNodeLabel(IGetNodeID(*vp.first)));
				nodeIds[IGetNodeID(*vp.first)] = target_nodeId;
			}
			else{
				target_nodeId = nodeIds[IGetNodeID(*vp.first)];
			}
			// cout << "Insert Edge " << ori_source_id << " " << IGetNodeID(*vp.first) << endl;
			g->BInsertEdge(source_nodeId, target_nodeId, IGetEdgeLabel(EGetEdge(ori_source_id, IGetNodeID(*vp.first))));
			if(!visited[IGetNodeID(*vp.first)])
			{	
				DFS(IGetNodeID(*vp.first), visited, nodeIds, g);
			}
		}
		// else{
			// cout << "YES HIDDEDN EDGE btw " << ori_source_id << " " << IGetNodeID(*vp.first) << endl;
		// }
		++vp.first;
	}
}
			...
			if(findIfExist(knownGraphsMap, r, c))
			{
				for(int j=0; j < knownGraphsMap[r][c]->knownFreqGraphs.size(); j++)
				{
					...
					if(g.BIsExactlyEqualTo(knownGraphsMap[r][c]->knownFreqGraphs[j])) // it is frequent
					{
						...
						// cout << "Here or what" << endl;
						Graph *temp_g = new Graph();
						temp_g->Copy(&g);
						for(int k=0;k<(*ansGraphs).size();k++)
						{
							delete (*ansGraphs)[k];
						}
						(*ansGraphs).clear();
				...
				// cout << "Size start: " << (*ansGraphs).size() << endl;
				...
				// cout << "Size end: " << (*ansGraphs).size() << endl;
			}
...
vector<Graph*> Graph::GetComponents(Graph *g)
{
	string cand_code = this->GetNewGraphCode();
	string db_code = g->GetNewGraphCode();
	vector<Graph*> temp;
	...
	GraphType type_gt = GetGraphType();
	if(type_gt == GRAPH_EMPTY || type_gt == GRAPH_SINGLE_NODE)
	{
		temp.push_back(tempGraph);
		return temp;
	}
	// cout << "CAND Graph Code: " << cand_code << endl;
	// cout << "DB Graph Code: " << db_code << endl;

	stringstream cand(cand_code);
    vector<int> cand_codes(0);
    string tmp;
    while(getline(cand, tmp, '.')){
		int tmp1 = atoi((char*)tmp.c_str());
        cand_codes.push_back(tmp1);
    }

	stringstream db(db_code);
    vector<int> db_codes(0);
    while(getline(db, tmp, '.')){
		int tmp1 = atoi((char*)tmp.c_str());
        db_codes.push_back(tmp1);
    }

	// string unique_cand_code = cand_code.substr(0, 6);
	vector<int> unique_cand_code(cand_codes.begin(), cand_codes.begin()+3);
	int count=0;
	int uniq_size = 3;

	for(int i=3;i<cand_codes.size()-2;i+=3){
		if(unique_cand_code[uniq_size - 3]!=cand_codes[i] || unique_cand_code[uniq_size - 2]!=cand_codes[i + 1] || unique_cand_code[uniq_size - 1]!=cand_codes[i + 2])
		{
			count=0;
			while(count<3){
				unique_cand_code.push_back(cand_codes[i+count]);
				count++;
			}
			// unique_cand_code += cand_code.substr(i, 6);
			uniq_size+=3;
		}
	}
	// cout << "UNIQUE CAND CODE " << unique_cand_code << endl;
	// cout << "UNIQUE CAND CODE " << endl;
	vector<int>::iterator it;
	// it = unique_cand_code.begin();
	// while(it!=unique_cand_code.end()){
	// 	cout << *it << " ";
	// 	it++;
	// }
	// cout << endl;

	// string unique_db_code = db_code.substr(0,6);
	vector<int> unique_db_code(db_codes.begin(), db_codes.begin()+3);
	uniq_size = 3;
	for(int i=3;i<db_codes.size()-2;i+=3){
		if(unique_db_code[uniq_size - 3]!=db_codes[i] || unique_db_code[uniq_size - 2]!=db_codes[i + 1] || unique_db_code[uniq_size - 1]!=db_codes[i + 2])
		{
			count=0;
			while(count<3){
				unique_db_code.push_back(db_codes[i+count]);
				count++;
			}
			// unique_db_code += db_code.substr(i, 6);
			uniq_size+=3;
		}
	}
	// cout << "UNIQUE DB CODE " << unique_db_code << endl;
	// cout << "UNIQUE DB CODE " << endl;
	// it = unique_db_code.begin();
	// while(it!=unique_db_code.end()){
	// 	cout << *it << " ";
	// 	it++;
	// }
	// cout << endl;
	...
	// string remove_edge = "";
	vector<int> remove_edge(0);
	int check_index=0;

	for(int i=0;i<unique_cand_code.size()-2;i+=3){
		if(check_index >= unique_db_code.size()){
			count=0;
			while(count<3){
				remove_edge.push_back(unique_cand_code[i+count]);
				count++;
			}
			// remove_edge += unique_cand_code.substr(i,6);
			continue;
		}
		
		if(unique_cand_code[i]!=unique_db_code[check_index] || unique_cand_code[i+1]!=unique_db_code[check_index+1] || unique_cand_code[i+2]!=unique_db_code[check_index+2])
		{
			if(unique_cand_code[i]!=unique_db_code[check_index]){
				if(unique_cand_code[i] < unique_db_code[check_index]){
					// remove_edge += unique_cand_code.substr(i,6);
					count=0;
					while(count<3){
			...
			else if(unique_cand_code[i+1]!=unique_db_code[check_index+1]) {
				if(unique_cand_code[i+1] < unique_db_code[check_index+1]){
					// remove_edge += unique_cand_code.substr(i,6);
					count=0;
					while(count<3){
						remove_edge.push_back(unique_cand_code[i+count]);
				...
				if(unique_cand_code[i+2] < unique_db_code[check_index+2]){
					// remove_edge += unique_cand_code.substr(i,6);
					count=0;
					while(count<3){
						remove_edge.push_back(unique_cand_code[i+count]);
	...
	// cout << "REMOVE THESE EDGES " << remove_edge << " of size " << remove_edge.size() << endl;
	// cout << "REMOVE EDGES " << endl;
	// it = remove_edge.begin();
	// while(it!=remove_edge.end()){
	// 	cout << *it << " ";
	// 	it++;
	// }
	// cout << endl;
	...
	{
		// PrintEdge(*ei);
		// cout << IGetSourceNode(*ei) << endl;
		int source_node = IGetSourceNode(*ei);
		int target_node = IGetTargetNode(*ei);
		int l1 = nodeLabels_Vi[source_node];
		int l2 = nodeLabels_Vi[target_node];
		...
		// 	// code += '0' + l1;
		// }
		// cout << "Edge String: " << code << endl;
		// cout << "Check size: " << remove_edge.size()-5 << endl;
		if(l1>l2)
		{
			int temp1 = l1;
			l1 = l2;
		...
		int size_rem = remove_edge.size();
		for(int i=0;i<size_rem-2;i+=3){
			if(remove_edge[i] == l1 && remove_edge[i+1] == el && remove_edge[i+2] == l2)
			{
				// cout << "Hiding Edge " << l1 << " " << el << " " << l2 << endl; 
				// PrintEd/ge(*ei);
				tempGraph->HideEdge(tempGraph->EGetEdge(source_node, target_node));
				if(!tempGraph->BIsValidNode(source_node))
					tempGraph->DecrementLabelFreq(source_node);
				if(!tempGraph->BIsValidNode(target_node)) // else if??
					tempGraph->DecrementLabelFreq(target_node);
				// cout << "HIDDEN or What? " << tempGraph->BIsThisEdgeHidden(source_node, target_node) << endl;
				// PrintEdge(*ei);
				// cout << "Hided Edge " << l1 << " " << el << " " << l2 << endl; 
				break;
			}
		}
	}
	...
	int n = this->IGetNoOfAllNodes();
	vector<int> visited = vector<int>(n, 0);
	vector<int> node_Ids = vector<int>(n, -1);
	for(int i=0; i< num_vertices(G); i++)
	{
		if(!visited[i])
		{
			Node source_node = allNodes_Vn[i];
			Graph* g1 = new Graph();
			g1->noOfValidNodes_i = 0;
			// cout << g1->noOfValidNodes_i << endl;
			int source_id = g1->BInsertNode(IGetNodeLabel(IGetNodeID(source_node)));
			// cout << "CHECK THIS " << source_id << endl;
			node_Ids[IGetNodeID(source_node)] = source_id;
			tempGraph->DFS(IGetNodeID(source_node),visited, node_Ids, g1);
			// tempGraph->DFS(source_id,visited, node_Ids, g1, IGetNodeID(source_node));
			GraphType type_g1 = g1->GetGraphType();
			if(type_g1 == GRAPH_EMPTY)
			{
				delete g1;
				continue;
			}
			...
			bool flag = true;
			while(git!=temp.end()){
				if(g1->BIsSubGraphOf(*git)){
					flag = false;
				}
				else if((*git)->BIsSubGraphOf(g1)){
					delete *git;
					temp.erase(git);
					git--;
					// k--;
...
void Graph::GetOnlyOneParent(Graph *parent_Pg)
{
	...
	if(type_gt == GRAPH_EMPTY)
	{
		...
		for(int i=parents_Pg.size()-1;i>=0;i--)
		{
			delete parents_Pg[i];
		}
		parents_Pg.erase(parents_Pg.begin(), parents_Pg.end());
		vector<Graph*>().swap(parents_Pg);
		return;
	}
	if( type_gt == GRAPH_SINGLE_NODE)
	{
	...
	// collect all the edges that can be restored
	// cout << "Here: " << hiddenEdges_Ve.size() << endl;
...
vector<Graph*> Graph::GetChildren(int *noOfChildren)
{
	...
	for(int i=0;i<numChild;i++)
	{
		...
		// cout << "HEREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE" << endl;
	}
...
vector<Graph*> Graph::GetChildrenOfSingleNodeGraph(int *noOfChildren)
{
	...
	Graph* g = new Graph();
	children_Pg.push_back(g);
	// cout << "HEREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE SN" << endl;
...
vector<Graph*> Graph::GetChildrenOfSingleEdgeGraph(int *noOfChildren)
{
	...
	Graph* g1 = new Graph();
	children_Pg.push_back(g);
	// cout << "HEREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE SE1" << endl;

	children_Pg.push_back(g1);
	// cout << "HEREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE SE2" << endl;
...
bool Graph::BIsSubGraphOf(Graph *super_g)
{
	...
	for(int i=0; i < superNodes_Vn.size(); i++)
	{
		...
		//std::cout << "/* 4message */" << '\n';
		if(IGetDegree(IGetNodeID(start_n)) > super_g->IGetDegree(superNodes_Vn[i]))
		{
			cout << "Here anytmie" << endl;
			continue;
		}
...
bool Graph::GetUnion(Graph *g)
{
	// this < g in terms of edges
    // this => sub
    // g => super

	// Get union of this and g and store in this
	GraphType type_gt = GetGraphType();
	if(g->GetGraphType() == GRAPH_EMPTY)
		return true;

	if(g->GetGraphType() == GRAPH_SINGLE_NODE)
	{
		int super_n = BInsertNode(g->IGetNodeLabel(g->IGetNodeID(NGetFirstValidNode())));
		int sub_n = IGetNodeID(NGetFirstValidNode());
		BInsertEdge(sub_n, super_n, 1);
		return true;
	}

	// for all the vertices get the one which gives maximum edge matching
	int max_match2=0;
	map<int, int> f_subToSuper_m, f_superToSub_m;
	set<Edge> f_visited_Se, f_toBeVisited_Se;
	...
	for(int j=0;j<num_vertices(G);j++)
	{
		...
		Node start_n = allNodes_Vn[j];

		map<int, int> f1_subToSuper_m, f1_superToSub_m;
		set<Edge> f1_visited_Se, f1_toBeVisited_Se;

		int max_match=0;
		vector<Node> gNodes_Vn = g->VNGetNodesWithLabel(nodeLabels_Vi[IGetNodeID(start_n)]);
		for(int i=0; i < gNodes_Vn.size(); i++)
		{
			// this is the recursive function for matching 'this' and
			// 'g' starting at the start_n

			map<int, int> subToSuper_m, superToSub_m;
			set<Edge> visited_Se, toBeVisited_Se;

			// do the mapping
			int v1 = IGetNodeID(start_n);
			int v2 = g->IGetNodeID(gNodes_Vn[i]);

			subToSuper_m.insert({v1,v2});
			superToSub_m.insert({v2,v1});

			int val12 = ICanBeMatchedWith(g, start_n, gNodes_Vn[i], subToSuper_m, superToSub_m, visited_Se, toBeVisited_Se);
			if(val12 >= max_match)
			{
				max_match = val12;
				f1_subToSuper_m.clear();
				f1_superToSub_m.clear();
				f1_subToSuper_m.insert(subToSuper_m.begin(), subToSuper_m.end());
				f1_superToSub_m.insert(superToSub_m.begin(), superToSub_m.end());
				f1_visited_Se = visited_Se;
				f1_toBeVisited_Se = toBeVisited_Se;				
			}
		}

		map<int, int>::iterator it1;
		it1 = f1_superToSub_m.begin();
		// cout << "Temp Mapping Result" << endl;
		// cout << "No of matching edges: " << max_match << endl;
		// while(it1!=f1_superToSub_m.end()){
		// 	cout << it1->first << " " << it1->second << endl;
		// 	it1++;
		// }
		// cout << "Temp ended here" << endl;

		if(max_match >= max_match2)
		{
			max_match2 = max_match;
			f_subToSuper_m.clear();
			f_superToSub_m.clear();
			f_subToSuper_m.insert(f1_subToSuper_m.begin(), f1_subToSuper_m.end());
			f_superToSub_m.insert(f1_superToSub_m.begin(), f1_superToSub_m.end());
			f_visited_Se = f1_visited_Se;
			f_toBeVisited_Se = f1_toBeVisited_Se;				
		}
	}

	if(max_match2 ==0 || max_match2 < IGetNoOfAllEdges()/2)
	{
		// cout << "Here 0" << endl; 
		return false;
	}
	...
	f_toBeVisited_Se.clear();

	map<int, int>::iterator it1;
	it1 = f_superToSub_m.begin();
	// cout << "Mapping Result" << endl;
	// cout << "No of matching edges: " << max_match2 << endl;
	// while(it1!=f_superToSub_m.end()){
	// 	cout << it1->first << " " << it1->second << endl;
	// 	it1++;
	// }
	...
	edge_iterator ei, ei_end;
	for (tie(ei, ei_end) = g->PEGetAllEdges(); ei != ei_end; ++ei)
	{
		Edge edge1 = *ei;
		set<Edge>::iterator find_SeIt = f_visited_Se.find(edge1);
		if(find_SeIt == f_visited_Se.end()){ //if its not present in visited edges
			f_toBeVisited_Se.insert(edge1);
		}
	}

	set<Edge>::iterator it;
	it = f_toBeVisited_Se.begin();
	while(it!=f_toBeVisited_Se.end())
	{
		Edge next_e = *it;
		// g->PrintEdge(next_e);
		// cout << endl;
		int v1 = g->IGetSourceNode(next_e);
		int v2 = g->IGetTargetNode(next_e);
		// cout << v1 << " " << v2 << endl;

		int sub_v1, sub_v2;
		map<int, int>::iterator findNext_mIt = f_superToSub_m.find(v1);
		if(findNext_mIt != f_superToSub_m.end())
		{
			sub_v1 = (*findNext_mIt).second;
			findNext_mIt = f_superToSub_m.find(v2);
			if(findNext_mIt != f_superToSub_m.end())
				sub_v2 = (*findNext_mIt).second;
			else
				sub_v2 = BInsertNode(g->IGetNodeLabel(v2));
		}
		else
		{
			sub_v1 = BInsertNode(g->IGetNodeLabel(v1));
			findNext_mIt = f_superToSub_m.find(v2);
			if(findNext_mIt != f_superToSub_m.end())
				sub_v2 = (*findNext_mIt).second;
			else
		...
		f_superToSub_m.insert({v1, sub_v1});
		f_superToSub_m.insert({v2, sub_v2});
		// cout << sub_v1 << " " << sub_v2 << endl;

		// cout << "Adding Edge: " << f_superToSub_m[v1] << " " <<  f_superToSub_m[v2] << " " <<  IGetEdgeLabel(next_e) << endl;
		BInsertEdge(f_superToSub_m[v1], f_superToSub_m[v2], IGetEdgeLabel(next_e));
		it++;
	}
	if(max_match2 == 0)
	{
		int sub_n = IGetNodeID(NGetFirstValidNode());
		int super_n = g->IGetNodeID(NGetFirstValidNode());
		cout << "Adding 1 edge" << endl;
		BInsertEdge(sub_n, f_superToSub_m[super_n], 1);
	}
...
bool Graph::GetSoftUnion(Graph *g)
{
	// this < g in terms of edges
    // this => sub
    // g => super
...
// return the number of edges matched
int Graph::ICanBeMatchedWith(Graph *g, Node subCurrent_n, Node superCurrent_n,
		map<int, int> &subToSuper_m, map<int, int> &superToSub_m,
		set<Edge> &visited_Se, set<Edge> &toBeVisited_Se)
{
	// cout << "ICanBeMatchedWith: " << IGetNodeID(subCurrent_n) << " " << g->IGetNodeID(superCurrent_n) << endl;
	// add all the edges from the 'subCurrent_n' to the 'toBeVisited_Se'
	pair<out_edge_iterator, out_edge_iterator> e_iterator = boost::out_edges(subCurrent_n, G);
	...
	// check if all the edges have been visited and all the nodes have been mapped
	if(toBeVisited_Se.size() == 0){
		// cout << "Nothing left to visit 1" << endl;
		return 0;
	}
	...
	// cout << "Working on this edge" << endl;
	// PrintEdge(next_e);
	// cout << endl;
	...
	{
		subFrom_n = NGetNodeWithID(IGetSourceNode(next_e));
		superFrom_n = g->NGetNodeWithID((*findNext_mIt).second);
	}
	else
	{
		findNext_mIt = subToSuper_m.find(IGetTargetNode(next_e));
	...
	/*
	 * Get all the nodes in supergraph which are adjacent to superCurrent_n
	 * and which have the same label as 'subTo_n'
	 */
	vector<Node> superTo_Vn = g->VNGetNodesAdjToWithLabel(superFrom_n,
			nodeLabels_Vi[IGetNodeID(subTo_n)],IGetEdgeLabel(next_e));
	
	if(superTo_Vn.size() == 0){
		map<int, int>::iterator it1;
		it1 = superToSub_m.begin();
		// cout << "Temp Mapping Result" << endl;
		// while(it1!=superToSub_m.end()){
		// 	cout << it1->first << " " << it1->second << endl;
		// 	it1++;
		// }
		// cout << "Temp ended here" << endl;
		// cout << "Nothing left to visit 2" << endl;
		return 0;
	}
	/*
	 * for each element of superTo_Vn: s
	 * if s is not mapped or if its mapped to subTo_n
	 * then map s to subTo_n
	 */
	map<int, int>::iterator findSub_mIt = subToSuper_m.find(IGetNodeID(subTo_n));
	
	int max_match=0, f_nodeID_matched = 0;
	map<int, int> f_subToSuper_m, f_superToSub_m;
	set<Edge> f_visited_Se, f_toBeVisited_Se;
	...
	for(int i=0; i < superTo_Vn.size(); i++)
	{
		...
		bool shouldBeMapped_b = false;
		// find superTo_Vn[i] in subToSuper_m
		if(findSub_mIt == subToSuper_m.end()) // not mapped
		{
			map<int, int>::iterator findSuper_mIt = superToSub_m.find( g->IGetNodeID(superTo_Vn[i]) );
			if(findSuper_mIt == superToSub_m.end()) // both are not mapped
				shouldBeMapped_b = true;
		}
		// subTo_n is already mapped
		// and its should be mapped to superTo_Vn[i]
		else if( (*findSub_mIt).second == g->IGetNodeID(superTo_Vn[i]))
			shouldBeMapped_b = true;

		if(shouldBeMapped_b)
		{
			// do the mapping
			int v1 = IGetNodeID(subTo_n);
			int v2 = g->IGetNodeID(superTo_Vn[i]);

			subToSuper_m.insert({v1,v2});
			superToSub_m.insert({v2,v1});

			int val123 = this->ICanBeMatchedWith(g, subTo_n, superTo_Vn[i], subToSuper_m, superToSub_m, visited_Se, toBeVisited_Se);
			if(val123 >= max_match)
			{
				max_match = val123;
				f_nodeID_matched = i;
				f_subToSuper_m.clear();
				f_superToSub_m.clear();
				f_subToSuper_m.insert(subToSuper_m.begin(), subToSuper_m.end());
				f_superToSub_m.insert(superToSub_m.begin(), superToSub_m.end());
				f_visited_Se = visited_Se;
				f_toBeVisited_Se = toBeVisited_Se;				
			}
			
			subToSuper_m.erase(v1);
			superToSub_m.erase(v2);	
		}
	}

	// do the mapping
	int v1 = IGetNodeID(subTo_n);
	int v2 = g->IGetNodeID(superTo_Vn[f_nodeID_matched]);

	subToSuper_m = f_subToSuper_m;
	superToSub_m = superToSub_m;
	visited_Se = f_visited_Se;
	toBeVisited_Se = f_toBeVisited_Se;	
...
bool Graph::GetCommonChild(Graph *g1, Graph *g2)
{
    ...
    if(g1_gt == GRAPH_SINGLE_NODE)
    {
		...
		delete children_Pg[0];
		children_Pg.erase(children_Pg.begin(), children_Pg.end());
		vector<Graph*>().swap(children_Pg);
        return true;
    }
